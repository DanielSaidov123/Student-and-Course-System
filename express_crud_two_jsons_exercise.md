...
app.get("/stats", async (req, res) => {
  ...
  const coursesWithCount = courses.map((course) => ({
    ...course,
    enrolledCount: students.filter((s) => s.enrolledCourses.includes(course.id))
      .length,
  }));

  const mostPopularCourse = coursesWithCount.reduce(
    (max, course) =>
  ...
  const mostActiveStudent = students.reduce(
    (max, student) =>
      student.enrolledCourses.length > (max?.enrolledCourses.length || 0)
        ? student
        : max,
    null
  );
